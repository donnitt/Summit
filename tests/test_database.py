...
class DatabaseTest(unittest.TestCase):
    ...
    def test_pin_is_disabled_by_default(self) -> None:
        self.assertFalse(self.database.is_pin_enabled())
        self.assertFalse(self.database.verify_pin("1234"))

    def test_set_pin_enables_and_verifies_correctly(self) -> None:
        self.database.set_pin("1234")
        self.assertTrue(self.database.is_pin_enabled())
        self.assertTrue(self.database.verify_pin("1234"))
        self.assertFalse(self.database.verify_pin("0000"))

    def test_set_pin_rejects_invalid_format(self) -> None:
        with self.assertRaises(ValueError):
            self.database.set_pin("12")
        with self.assertRaises(ValueError):
            self.database.set_pin("abcd")

    def test_disable_pin_clears_protection(self) -> None:
        self.database.set_pin("1234")
        self.database.disable_pin()
        self.assertFalse(self.database.is_pin_enabled())
        self.assertFalse(self.database.verify_pin("1234"))

    def test_pin_persists_across_reopen(self) -> None:
        self.database.set_pin("5678")
        reopened = Database(self.database.path)
        self.assertTrue(reopened.is_pin_enabled())
        self.assertTrue(reopened.verify_pin("5678"))

    def test_changing_pin_invalidates_previous_pin(self) -> None:
        self.database.set_pin("1111")
        self.database.set_pin("2222")
        self.assertFalse(self.database.verify_pin("1111"))
        self.assertTrue(self.database.verify_pin("2222"))
