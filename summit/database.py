...
DEFAULT_CATEGORIES = (
    "Alimentação", "Transporte", "Moradia", "Contas", "Saúde", "Autocuidado",
    "Lazer", "Educação", "Compras", "Assinaturas", "Materiais", "Marketing",
    "Espaço", "Serviços", "Salário", "Contratos", "Impostos", "Outros",
)
...
class Database:
    def __init__(self, path: Path | str | None = None) -> None:
        ...
        # Uma única conexão persistente, reaproveitada por todos os métodos
        # (evita abrir/fechar um arquivo sqlite a cada consulta — antes cada
        # chamada como accounts(), transactions(), etc. abria sua própria
        # conexão, e o snapshot() sozinho já disparava ~10 delas).
    ...
    def close(self) -> None:
        """Fecha a conexão persistente. Chamar ao encerrar o app."""
    ...
    def _initialize(self) -> None:
        with self.connect() as connection:
            connection.executescript(
                ...
                CREATE INDEX IF NOT EXISTS idx_imported_status ON imported_transactions(status);
                """
            )
            ...
            self._ensure_column(connection, "app_settings", "pin_enabled", "INTEGER NOT NULL DEFAULT 0")
            self._ensure_column(connection, "app_settings", "pin_hash", "TEXT NOT NULL DEFAULT ''")
            self._ensure_column(connection, "app_settings", "pin_salt", "TEXT NOT NULL DEFAULT ''")
            ...
            # Recupera categorias já usadas em lançamentos antigos, mas que
            # ainda não constam no catálogo (ex.: bancos migrados).
            for table in ("transactions", "fixed_expenses", "fixed_incomes", "debts", "budgets"):
                for row in connection.execute(f"SELECT DISTINCT category FROM {table}"):
                    self._ensure_category(connection, row["category"])
    ...
    @staticmethod
    def _ensure_category(connection: sqlite3.Connection, name: str) -> None:
        """Register a category name in the catalog if it isn't there yet.

        Called whenever a lançamento is saved with a category typed by hand,
        so it becomes available as a suggestion next time without any extra
        step from the user.
        """
        cleaned = (name or "").strip()
        if cleaned:
            connection.execute("INSERT OR IGNORE INTO categories (name) VALUES (?)", (cleaned,))
    ...
    def settings(self) -> dict[str, Any]:
        with self.connect() as connection:
            ...
            if row:
                return dict(row)
            return {
                "id": 1, "theme_mode": "dark", "accent_color": "#8562ef",
                "pin_enabled": 0, "pin_hash": "", "pin_salt": "",
            }
    ...
    def is_pin_enabled(self) -> bool:
        with self.connect() as connection:
            row = connection.execute("SELECT pin_enabled FROM app_settings WHERE id = 1").fetchone()
            return bool(row and row["pin_enabled"])

    def set_pin(self, pin: str) -> None:
        """Create or replace the lock PIN with a freshly generated salt."""
        cleaned = self._validate_pin_format(pin)
        salt = os.urandom(16)
        pin_hash = self._hash_pin(cleaned, salt)
        with self.connect() as connection:
            connection.execute(
                """
                INSERT INTO app_settings (id, pin_enabled, pin_hash, pin_salt)
                VALUES (1, 1, ?, ?)
                ON CONFLICT(id) DO UPDATE SET
                    pin_enabled = 1, pin_hash = excluded.pin_hash, pin_salt = excluded.pin_salt
                """,
                (pin_hash, salt.hex()),
            )

    def verify_pin(self, pin: str) -> bool:
        """Compare a typed PIN against the stored hash using constant-time comparison."""
        cleaned = (pin or "").strip()
        with self.connect() as connection:
            row = connection.execute(
                "SELECT pin_hash, pin_salt FROM app_settings WHERE id = 1"
            ).fetchone()
        if not row or not row["pin_salt"] or not row["pin_hash"]:
            return False
        salt = bytes.fromhex(row["pin_salt"])
        candidate = self._hash_pin(cleaned, salt)
    ...
    def disable_pin(self) -> None:
        with self.connect() as connection:
            connection.execute(
                ...
                INSERT INTO app_settings (id, pin_enabled, pin_hash, pin_salt)
                VALUES (1, 0, '', '')
                ON CONFLICT(id) DO UPDATE SET pin_enabled = 0, pin_hash = '', pin_salt = ''
                """
            )
    ...
    def backup_to(self, destination: str | Path) -> Path:
        ...
        with self.connect() as connection:
            # Como agora mantemos uma conexão persistente (modo WAL), é
            # preciso forçar o checkpoint explicitamente antes de copiar o
            # arquivo — sem isso, dados recentes poderiam ficar só no
            # summit.db-wal e não seriam incluídos no backup.
    ...
    def categories(self) -> list[str]:
        with self.connect() as connection:
            return [
                row["name"]
                for row in connection.execute("SELECT name FROM categories ORDER BY name COLLATE NOCASE")
            ]
    ...
    def add_category(self, name: str) -> None:
        ...
        with self.connect() as connection:
            existing = connection.execute(
                "SELECT 1 FROM categories WHERE name = ? COLLATE NOCASE", (cleaned,)
            ).fetchone()
            if existing:
                raise ValueError("Essa categoria já existe.")
            connection.execute("INSERT INTO categories (name) VALUES (?)", (cleaned,))

    def rename_category(self, old_name: str, new_name: str) -> None:
        cleaned = self._required_text(new_name, "categoria")
        try:
            with self.connect() as connection:
                if cleaned.casefold() != old_name.casefold():
                ...
                cursor = connection.execute(
                    "UPDATE categories SET name = ? WHERE name = ? COLLATE NOCASE", (cleaned, old_name)
                )
                self._ensure_updated(cursor, "Categoria não encontrada.")
                # Mantém os lançamentos já existentes coerentes com o novo nome.
                for table in ("transactions", "fixed_expenses", "fixed_incomes", "debts"):
                    connection.execute(
                        f"UPDATE {table} SET category = ? WHERE category = ? COLLATE NOCASE", (cleaned, old_name)
                    )
                connection.execute(
                    "UPDATE budgets SET category = ? WHERE category = ? COLLATE NOCASE", (cleaned, old_name)
                )
        except sqlite3.IntegrityError as error:
            raise ValueError("Já existe um orçamento com o novo nome de categoria.") from error

    def delete_category(self, name: str) -> None:
        with self.connect() as connection:
            cursor = connection.execute("DELETE FROM categories WHERE name = ? COLLATE NOCASE", (name,))
            self._ensure_updated(cursor, "Categoria não encontrada.")
    ...
    def add_transaction(
        ...
        with self.connect() as connection:
            ...
            self._ensure_category(connection, values[1])
    ...
    # -- Importação de extratos (conciliação bancária) -----------------

    def import_bank_rows(self, rows: list[Any], account_id: int, scope: str = "work") -> dict[str, int]:
        """Recebe candidatos já normalizados (ImportedRow) e grava em staging.

        Linhas com o mesmo dedup_hash de uma importação anterior são
        ignoradas silenciosamente (evita duplicar ao reimportar o mesmo
        arquivo). Linhas que batem com um lançamento manual já existente
        (mesma conta, data e valor) são marcadas como possível duplicata
        para o usuário decidir na tela de revisão.
        """
        inserted = 0
        skipped = 0
        with self.connect() as connection:
            for row in rows:
                existing = connection.execute(
                    ...
                    WHERE account_id = ? AND transaction_date = ? AND amount = ? AND kind = ?
                    """,
                    (account_id, row.transaction_date, row.amount, row.kind),
                ).fetchone()
                cursor = connection.execute(
                    """
                    INSERT OR IGNORE INTO imported_transactions
                        (description, category, amount, kind, transaction_date, account_id,
                         scope, source, dedup_hash, possible_duplicate)
                    VALUES (?, ?, ?, ?, ?, ?, ?, 'csv', ?, ?)
                    """,
                    (
                        row.description, row.category, row.amount, row.kind, row.transaction_date,
                        account_id, scope, row.dedup_hash, 1 if existing else 0,
                    ),
                )
                if cursor.rowcount:
                    inserted += 1
    ...
    def pending_imports(self) -> list[dict[str, Any]]:
        with self.connect() as connection:
            rows = connection.execute(
                """
                SELECT imported_transactions.*, accounts.name AS account_name
                FROM imported_transactions
                LEFT JOIN accounts ON accounts.id = imported_transactions.account_id
                WHERE imported_transactions.status = 'pending'
                ORDER BY imported_transactions.transaction_date DESC, imported_transactions.id DESC
                """
            ).fetchall()
    ...
    def confirm_import(
        ...
        """Confirma um candidato: cria o lançamento definitivo e marca a
        linha de staging como concluída, num único commit."""
        values = self._transaction_values(
            description, category, amount, kind, status, transaction_date, account_id, False, notes, scope
        )
        with self.connect() as connection:
            cursor = connection.execute(
                "SELECT 1 FROM imported_transactions WHERE id = ? AND status = 'pending'",
                (int(import_id),),
            )
            if cursor.fetchone() is None:
                raise ValueError("Este item já foi revisado.")
            connection.execute(
                """
                INSERT INTO transactions
                    (description, category, amount, kind, status, transaction_date, account_id, recurring, notes, scope)
            ...
            self._ensure_category(connection, values[1])
            connection.execute(
                "UPDATE imported_transactions SET status = 'confirmed' WHERE id = ?", (int(import_id),)
            )

    def ignore_imports(self, import_ids: list[int]) -> None:
        if not import_ids:
            return
        with self.connect() as connection:
            connection.executemany(
                "UPDATE imported_transactions SET status = 'ignored' WHERE id = ? AND status = 'pending'",
                [(int(import_id),) for import_id in import_ids],
            )
    ...
    def add_debt(
        ...
        with self.connect() as connection:
            ...
            self._ensure_category(connection, values[2])
    ...
    def add_budget(self, category: str, monthly_limit: float, notes: str = "") -> None:
        ...
        try:
            with self.connect() as connection:
                ...
                self._ensure_category(connection, values[0])
        except sqlite3.IntegrityError as error:
