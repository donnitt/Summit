...
QPushButton#TitleBarButton {{
    background: #1b1524; border: 1px solid #2f2739; border-radius: 6px;
    color: #d8d0e2; font-size: 14px; font-weight: 600;
}}
QPushButton#TitleBarButton:hover {{ background: #2b2140; border-color: #3d2f52; color: #ffffff; }}
QPushButton#TitleBarButton:pressed {{ background: #17111f; }}
QPushButton#TitleBarButtonClose {{
    background: #1b1524; border: 1px solid #2f2739; border-radius: 6px;
    color: #d8d0e2; font-size: 13px; font-weight: 600;
}}
QPushButton#TitleBarButtonClose:hover {{ background: #c23652; border-color: #c23652; color: #ffffff; }}
QPushButton#TitleBarButtonClose:pressed {{ background: #971f38; border-color: #971f38; }}
...
QLabel#ColumnHint {{ color: #948a9e; font-size: 10px; min-height: 25px; }}
...
QLabel#EmptyState {{
    color: #948a9e; font-size: 10px; background: #17111f;
...
QPushButton#Primary:hover {{ background: {accent_btn_hover}; }}
QPushButton#Primary:pressed {{ background: {accent_press}; }}
QPushButton#PagePrimary {{
    background: {accent}; color: {on_accent}; border: 0; border-radius: 10px;
    padding: 10px 17px; font-weight: 700;
}}
QPushButton#PagePrimary:hover {{ background: {accent_btn_hover}; }}
...
QLabel#PinDot {{ font-size: 24px; color: {accent}; }}
QLabel#PinError {{ color: #ef4444; font-size: 12px; font-weight: 600; }}
QPushButton#Keypad {{
    background: #1b1524; color: #f4f0f8; border: 1px solid #33283e;
    border-radius: 32px; font-size: 18px; font-weight: 600;
}}
QPushButton#Keypad:hover {{ border-color: {accent_border_strong}; color: white; }}
QPushButton#Keypad:pressed {{ background: {accent}; color: {on_accent}; border-color: {accent}; }}
QPushButton#CategoryLegendRow {{
    background: transparent; border: 0; border-radius: 7px; padding: 4px 6px; text-align: left;
}}
QPushButton#CategoryLegendRow:hover {{ background: #241d30; }}
QWidget#ThemeToggle {{ background: transparent; }}
QPushButton#SwitchButton {{ background: transparent; border: 0; padding: 0; margin: 0; }}
...
QPushButton#TextButton:hover {{ color: {accent_text_hover}; }}
...
QLineEdit[placeholderText], QTextEdit[placeholderText] {{ color: #948a9e; }}
...
QHeaderView::section {{ background: #130e1b; color: #948a9e; border: 0; padding: 10px; font-size: 10px; }}
...
QWidget#TitleBar {{ background: transparent; }}
QPushButton#TitleBarButton {{
    background: #ece6f9; border: 1px solid #ddd2f2; border-radius: 6px;
    color: #4a4456; font-size: 14px; font-weight: 600;
}}
QPushButton#TitleBarButton:hover {{ background: #ddd3f5; border-color: {accent_border}; color: #221f29; }}
QPushButton#TitleBarButton:pressed {{ background: #d3c6ef; }}
QPushButton#TitleBarButtonClose {{
    background: #ece6f9; border: 1px solid #ddd2f2; border-radius: 6px;
    color: #4a4456; font-size: 13px; font-weight: 600;
}}
QPushButton#TitleBarButtonClose:hover {{ background: #c23652; border-color: #c23652; color: #ffffff; }}
QPushButton#TitleBarButtonClose:pressed {{ background: #971f38; border-color: #971f38; }}
...
QLabel#BrandMark {{ background: {accent}; border-radius: 10px; font-size: 16px; font-weight: 800; color: {on_accent}; }}
QLabel#SectionLabel {{ color: #6f6579; font-size: 9px; font-weight: 700; letter-spacing: 1px; }}
QPushButton#NavButton {{
    border: 0; border-radius: 10px; padding: 11px 14px; text-align: left;
...
QLabel#Muted {{ color: #6f6579; }}
QLabel#Tiny {{ color: #6f6579; font-size: 10px; }}
...
QLabel#ColumnHint {{ color: #6f6579; font-size: 10px; min-height: 25px; }}
...
QLabel#FixedSection {{ color: #6f6579; font-size: 9px; font-weight: 700; letter-spacing: 1px; }}
QLabel#FixedSubtotal {{ color: #6e6479; font-size: 10px; padding-bottom: 2px; }}
QLabel#EmptyState {{
    color: #6f6579; font-size: 10px; background: #f7f4fc;
...
QPushButton#Secondary:checked {{ background: {accent}; color: {on_accent}; border-color: {accent}; }}
QLabel#PinDot {{ font-size: 24px; color: {accent}; }}
QLabel#PinError {{ color: #d92d3a; font-size: 12px; font-weight: 600; }}
QPushButton#Keypad {{
    background: #ffffff; color: #221f29; border: 1px solid #ddd6ea;
    border-radius: 32px; font-size: 18px; font-weight: 600;
}}
QPushButton#Keypad:hover {{ border-color: {accent_border_strong}; color: #221f29; }}
QPushButton#Keypad:pressed {{ background: {accent}; color: {on_accent}; border-color: {accent}; }}
QPushButton#CategoryLegendRow {{
    background: transparent; border: 0; border-radius: 7px; padding: 4px 6px; text-align: left;
}}
QPushButton#CategoryLegendRow:hover {{ background: #f2effa; }}
QWidget#ThemeToggle {{ background: transparent; }}
QPushButton#SwitchButton {{ background: transparent; border: 0; padding: 0; margin: 0; }}
...
QHeaderView::section {{ background: #f7f4fc; color: #6f6579; border: 0; padding: 10px; font-size: 10px; }}
QScrollArea {{ border: 0; background: transparent; }}
QScrollBar:vertical {{ background: transparent; width: 8px; margin: 2px; }}
...
def build_stylesheet(mode: str, accent: str = DEFAULT_ACCENT) -> str:
    ...
    tokens = {
        "accent": accent,
        # Button backgrounds always darken on hover (never lighten), so
        # {on_accent} text keeps enough contrast against the button fill.
        "accent_btn_hover": _shade(accent, -0.12),
        ...
        # Text-only hover color (TextButton/VisibilityToggle): on the dark
        # theme's near-black background a lighter tint reads as "brighter",
        # but that same lighter tint loses contrast on the light theme's
        # near-white background, so it must darken there instead.
        "accent_text_hover": _shade(accent, 0.20) if mode == "dark" else _shade(accent, -0.28),
    }
