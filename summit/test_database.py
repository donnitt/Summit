...
class DatabaseTest(unittest.TestCase):
    ...
    def test_category_catalog_autoregisters_rename_and_delete(self) -> None:
        self.database.setup_workspace("Doni", "Tatuador", "Conta", 0)
        self.assertIn("Alimentação", self.database.categories())

        self.database.add_transaction(
            "Sessão", "Fine line", 500, "income", "paid", "2026-09-09", 1
        )
        self.assertIn("Fine line", self.database.categories())

        self.database.add_category("Equipamentos")
        with self.assertRaises(ValueError):
            self.database.add_category("equipamentos")

        self.database.rename_category("Fine line", "Sessões")
        self.assertEqual(self.database.transactions()[0]["category"], "Sessões")
        self.assertNotIn("Fine line", self.database.categories())
        self.assertIn("Sessões", self.database.categories())

        self.database.delete_category("Sessões")
        self.assertNotIn("Sessões", self.database.categories())
        self.assertEqual(self.database.transactions()[0]["category"], "Sessões")

        with self.assertRaises(ValueError):
            self.database.delete_category("Categoria inexistente")
